...
if os.path.isfile(confidence_file) is False:
    ...
    filenumber = len(os.listdir("confidences/%s_wenda_gpu/" % args.prefix))
    print(filenumber)

    # Concatenate into one confidence file and save
    models = []
    for i in range(filenumber):
        filename = "confidences/%s_wenda_gpu/model_%d_confidence.txt" % (args.prefix, i)
...
for k in k_values:
    ...
    output_dir = os.path.join("output/%s_wenda_gpu/k_{0:02d}".format(k) % args.prefix)

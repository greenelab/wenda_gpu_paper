...
class WeightedElasticNet(glmnet.ElasticNet):
    ...
    def __init__(self, alpha, n_splits, lambda_path=None, standardize=True, scoring="mean_squared_error"):
    ...
    def fit(self, x, y, weights):
        ...
        if self.lambda_path is None:
            lambda_max = self._compute_lambda_max(x, y, weights)
        ...
        super().fit(x[train_indexes], y[train_indexes], relative_penalties=weights)
...
class Wenda:
    ...
    def _trainOneWeightedModel(self, group, penalty_weights, alpha, lambda_path, n_splits, predict_path, out=True):
        model = WeightedElasticNet(alpha, n_splits=n_splits, lambda_path=lambda_path, standardize=False)
        model.fit(self._x_train, self._y_train, penalty_weights)

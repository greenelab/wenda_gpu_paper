...
class TCGADataModel():
    ...
    def process_data_for_identifiers(self,
        ...
        if train_cancer_type != test_cancer_type:
            filebase = "data/pairwise/" + train_gene + "_" + train_cancer_type + "_" + test_cancer_type + "_" + signal
            os.makedirs(filebase, exist_ok=True)
            np.savetxt(os.path.join(filebase, "source_data.tsv"), self.X_train_raw_df, fmt="%.2f", delimiter="\t")
            np.savetxt(os.path.join(filebase, "target_data.tsv"), self.X_test_raw_df, fmt="%.2f", delimiter="\t")
            self.y_train_df.to_csv(os.path.join(filebase, "source_phenotypes.tsv"), sep="\t")
            self.y_test_df.to_csv(os.path.join(filebase, "target_phenotypes.tsv"), sep="\t")
            self.y_train_df['status'].to_csv(os.path.join(filebase, "source_y.tsv"), index=False, header=False)
            self.y_test_df['status'].to_csv(os.path.join(filebase, "target_y.tsv"), index=False, header=False)
